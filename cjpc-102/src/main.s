;
; Simple sprite demo for NES
; Copyright 2011-2014 Damian Yerrick
;
; Copying and distribution of this file, with or without
; modification, are permitted in any medium without royalty provided
; the copyright notice and this notice are preserved in all source
; code copies.  This file is offered as-is, without any warranty.
;

.include "nes.inc"
.include "global.inc"

.import fizzter_startoffsets, fizzter_tilerow0, fizzter_tilerow1
.import fizzter_tilerow2, fizzter_tilerow3
  

OAM = $0200

; Set TEST_DMC_DMA to nonzero to turn on a stress test of glitch
; avoidance in pads.s.  This emits a high-pitched tone.
TEST_DMC_DMA = 0

NEWLINE_WIDTH = 16

.segment "ZEROPAGE"
nmis:          .res 1
oam_used:      .res 1  ; starts at 0
cur_keys:      .res 1
new_keys:      .res 1
cur_throttle:  .res 1
camera_xsub:   .res 1
camera_x:      .res 2
throttle_digits: .res 3

glyph_column: .res 1
glyph_end: .res 1
have_new_column: .res 1
column_repeat: .res 1
text_pos: .res 2

.segment "CODE"
;;
; This NMI handler is good enough for a simple "has NMI occurred?"
; vblank-detect loop.  But sometimes there are things that you always
; want to happen every frame, even if the game logic takes far longer
; than usual.  These might include music or a scroll split.  In these
; cases, you'll need to put more logic into the NMI handler.
.proc nmi_handler
  inc nmis
  rti
.endproc

; A null IRQ handler that just does RTI is useful to add breakpoints
; that survive a recompile.  Set your debugging emulator to trap on
; reads of $FFFE, and then you can BRK $00 whenever you need to add
; a breakpoint.
;
; But sometimes you'll want a non-null IRQ handler.
; On NROM, the IRQ handler is mostly used for the DMC IRQ, which was
; designed for gapless playback of sampled sounds but can also be
; (ab)used as a crude timer for a scroll split (e.g. status bar).
.proc irq_handler
  rti
.endproc

.proc main

  ; Now the PPU has stabilized, and we're still in vblank.  Copy the
  ; palette right now because if you load a palette during forced
  ; blank (not vblank), it'll be visible as a rainbow streak.
  jsr load_main_palette

  ; While in forced blank we have full access to VRAM.
  ; Load the nametable (background map).
  jsr draw_bg
  
  ; Set up game variables, as if it were the start of a new level.
  jsr init_player

  ; Optional testing
  .if ::TEST_DMC_DMA
    lda #$4F  ; Looping playback
    sta $4010
    lda #$00  ; $C000, 1 byte
    sta $4012
    sta $4013
    lda #$1F  ; Begin playback
    sta $4015
  .endif

  lda #0
  sta camera_xsub
  sta camera_x+0
  sta camera_x+1
  sta glyph_column
  sta glyph_end
  sta have_new_column
  sta column_repeat
  lda #<display_text
  sta text_pos+0
  lda #>display_text
  sta text_pos+1

  ; Set sprite 0
  lda #183  ; y position
  sta OAM+0 ; x position
  sta OAM+3 ; y position
  lda #'_'
  sta OAM+1
  lda #$20  ; behind background
  sta OAM+2

forever:

  ; Copy the display list from main RAM to the PPU and then
  ; read the controllers.  (To see why these are done together,
  ; read pads.s.)
  jsr run_dma_and_read_pads

  ; Move the text
  ldx camera_x
  lda cur_throttle
  cmp #4
  bcs :+
    lda #4
  :
  pha
  asl a
  asl a
  asl a
  asl a
  clc
  adc camera_xsub
  sta camera_xsub
  pla
  php
  lsr a
  lsr a
  lsr a
  lsr a
  plp
  adc camera_x+0
  sta camera_x+0
  lda camera_x+1
  adc #0
  sta camera_x+1

  ; Does a new column of text need to be drawn?
  txa
  eor camera_x+0
  and #$F8
  sta have_new_column
  beq no_new_glyph
  lda glyph_column
  cmp glyph_end
  bne no_new_glyph
    ldy #0
    lda (text_pos),y
    bne no_restart_text
      lda #<display_text
      sta text_pos+0
      lda #>display_text
      sta text_pos+1
      lda (text_pos),y
    no_restart_text:
    iny
    sty column_repeat
    inc text_pos+0
    bne :+
      inc text_pos+1
    :

    ; newline 
    cmp #' '
    bcs is_printable
      lda #NEWLINE_WIDTH-1
      sta column_repeat
      lda #' '
    is_printable:
      tay    
      lda fizzter_startoffsets-32,y
      sta glyph_column
      lda fizzter_startoffsets-31,y
      sta glyph_end
  no_new_glyph:

  jsr format_throttle

  ; Game logic goes here
  jsr move_player

  ; The first entry in OAM (indices 0-3) is "sprite 0".  In games
  ; with a scrolling playfield and a still status bar, it's used to
  ; help split the screen.  This demo doesn't use scrolling, but
  ; yours might, so I'm marking the first entry used anyway.  
  ldx #4
  stx oam_used
  ; Each routine adds to oam_used the number of bytes it wrote.
  jsr draw_player_sprite
  ldx oam_used
  jsr ppu_clear_oam

  ; Wait for sprite 0 if applicable
  lda #$C0
  :
    bit PPUSTATUS
    bvs :-
  :
    bit PPUSTATUS
    beq :-
  bmi skip_textbar
    lda camera_x+0
    tax
    and #$F8  ; don't change coarse X
    sta PPUSCROLL
    ; hblank should land between here...
    bit PPUSTATUS
    lda camera_x+1
    and #$01
    ora #VBLANK_NMI|BG_1000
    sta PPUCTRL
    ; ...and here
    stx PPUSCROLL
    bit PPUSTATUS
  skip_textbar:

  ; Good; we have the full screen ready.  Wait for a vertical blank
  ; and set the scroll registers to display it.
  lda nmis
vw3:
  cmp nmis
  beq vw3
  
  jsr draw_throttle

  lda have_new_column
  beq dont_copy_column
    lda camera_x+1
    and #$01
    asl a
    asl a
    eor #$27
    sta PPUADDR
    lda camera_x+0
    lsr a
    lsr a
    lsr a
    sta PPUADDR
    lda #VBLANK_NMI|VRAM_DOWN
    sta PPUCTRL
    ldx glyph_column
    lda fizzter_tilerow0,x
    sta PPUDATA
    lda fizzter_tilerow1,x
    sta PPUDATA
    lda fizzter_tilerow2,x
    sta PPUDATA
    lda fizzter_tilerow3,x
    sta PPUDATA
    dec column_repeat
    bne :+
      inc column_repeat
      inc glyph_column
    :
  dont_copy_column:

  ; Copy the display list from main RAM to the PPU
  ; (skipped: run_dma_and_read_pads already does this)
  
  ; Turn the screen on
  ldx #0
  ldy #0
  lda #VBLANK_NMI|BG_0000|OBJ_0000
  sec
  jsr ppu_screen_on
  jmp forever

.endproc

.proc load_main_palette
  ; seek to the start of palette memory ($3F00-$3F1F)
  ldx #$3F
  stx PPUADDR
  ldx #$00
  stx PPUADDR
copypalloop:
  lda initial_palette,x
  sta PPUDATA
  inx
  cpx #32
  bcc copypalloop
  rts
.endproc

.segment "RODATA"
initial_palette:
  .byt $22,$18,$28,$38,$0F,$06,$16,$26,$0F,$08,$19,$2A,$0F,$12,$0F,$0F
  .byt $22,$08,$16,$37,$0F,$06,$16,$26,$0F,$0A,$1A,$2A,$0F,$02,$12,$22

display_text:
  .incbin "src/rant.txt"
  .byte 0

; Include the CHR ROM data
.segment "CHR"
  .incbin "obj/nes/bggfx.chr"
  .incbin "obj/nes/spritegfx.chr"
