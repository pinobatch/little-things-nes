...
page2_msg:
  ...
  .byte "the bank mapped to $6000 and",10
  .byte "whether $6000 is ROM or RAM.",10
  .byte "Even if this register is set",10
  .byte "to RAM, the FME-7 still",10
  .byte "outputs the bank number on",10
  .byte "PRG A18-A13.  This was",10
  .byte "confirmed by l_oliveira on",10
  .byte "forums.nesdev.com, who",10
  .byte "rewired an FME-7 board to",10
  .byte "use a larger memory.",10
  .byte "[2/3]",0

page3_msg:
  .byte "Replacing the 6264 (8Kx8",10
  .byte "SRAM) on the NES-BTR or",10
  .byte "JSROM PCB with a 62256",10
  .byte "(32Kx8 SRAM) gives the NES",10
  .byte "program four 8K RAM banks",10
  .byte "to use.  This would provide",10
  .byte "ample space for a Z-machine",10
  .byte "or BASIC interpreter.",10
  .byte "",10
  .byte "",10
  .byte "",10
